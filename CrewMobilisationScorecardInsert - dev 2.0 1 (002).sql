...
WHERE [Crew Pool Status] = 'Active'
	...
	AND AA.[Mobilisation Cell] not in ('IND_F3_MOBILISE', 'IND_RECRUITMENT', 'IND_F9_MOBLISE', 'IND_F4_MOBILISE') -- not ind f3, ind rec, ind f9, índ f4 f3, ind rec, ind f9, índ f4
...
where 
	F.[Mobilisation Cell ID] not in ('VGRP00000117', 'VGRP00000118', 'VGR300000031', 'VGR400000265','VGRP00000105', 'GLAX00000023', 'GLAX00000010', 'GLAX00000005')   --removing palican, sea agency, seaway, ind f3, ind rec, ind f9, índ f4
...
WHERE 
    ...
    AND AA.[Mobilisation Cell ID] NOT IN ('VGRP00000105', 'GLAX00000023', 'GLAX00000010', 'GLAX00000005') -- not ind f3, ind rec, ind f9, índ f4
    AND EVT.[Crew ID] IS NULL
GROUP BY 
    AA.[Record Inserted On],
    AA.[Crew ID],
...
WHERE 
	...
	AND PD.[Mobilisation Cell ID] not in ('VGRP00000105', 'GLAX00000023', 'GLAX00000010', 'GLAX00000005') -- not ind f3, ind rec, ind f9, índ f4
	AND EVT.[Crew ID] IS NULL
...
SELECT 
	...
	AND coalesce(cpl.[Crew Pool ID], pd.[Mobilisation Cell ID]) not in ('VGRP00000105', 'GLAX00000023', 'GLAX00000010', 'GLAX00000005') -- not ind f3, ind rec, ind f9, índ f4
	AND EVT.[Crew ID] IS NULL
-------------------------------------
-- DOC Policy
...
from (
	...
	LEFT JOIN [ShipMgmt_Crewing].[tCrewEvents] EVT ON EVT.[Crew ID] =JJ.[Crew ID] and evt.[Crew Event Type] = 'Management Transfer'
...
FROM (
	...
	case when cast(ls.[Start Date] as date) > jjn.[Approved On] then cast(ls.[Start Date] as date) else jjn.[Approved On] end as [Approved On Date Final]
	 ...
	 Left join #lastvacationbeforejoiningagain ls on ls.[Crew ID] = jjn.[Crew ID]) ZZ
...
WHERE 
	...
	AND Mob.[Is Active] = 1
	AND mob.[Crew Pool ID] not in ('VGR400000297','VGRP00000105', 'GLAX00000023', 'GLAX00000010', 'GLAX00000005', --not romania, ind f3, ind rec, ind f9, índ f4
---------------------------------------
--Changed By: Tiffany Torres
--Changed On: 26/03/2025
